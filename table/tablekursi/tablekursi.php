...
if (isset($_POST["delete"]) && isset($_POST["deleteId"])) {
    foreach ($_POST["deleteId"] as $deleteId) {
        $sql = "DELETE from kursi where id_kursi=$deleteId";
        $query = $koneksi->query($sql);
    }
}
?>
...
<body>
    ...
    <div class="container mt-5">
        <header class="mb-4">
            <?php
            if ($data['role'] == 'admin') {
                echo '<a href="form/forminsertkursi.php">Insert New Data</a>';
            }
            ?>
        </header>
        <main>
            <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    <form action="" method="post">
                        ...
                        <tbody>
                            <?php
                            $query = "SELECT * FROM kursi"; // Ganti dengan nama tabel yang sesuai
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo "<tr>";
                                    if ($data['role'] == 'admin') {
                                        echo "<td><input type='checkbox' name='deleteId[]' value='" . $row['id_kursi'] . "'></td>";
                                    }
                                    ...
                                    if ($data['role'] == 'admin') {
                                        echo "<td><a href='form/formupdatekursi.php?id=" . $row['id_kursi'] . "' class='btn btn-warning btn-sm'>Edit</a> ";
                                    }
                                    echo "</tr>";
                                }
                            } else {

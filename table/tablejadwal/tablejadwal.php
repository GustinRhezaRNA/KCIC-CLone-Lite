...
if (isset($_POST["delete"]) && isset($_POST["deleteId"])) {
    foreach ($_POST["deleteId"] as $deleteId) {
        $sql = "DELETE from jadwal where id_jadwal=$deleteId";
        $query = $koneksi->query($sql);
    }
}
?>
...
<body>
    ...
    <div class="container mt-5">
        <header class="mb-4">
            <?php
            if ($data['role'] == 'admin') {
                echo '<a href="form/forminsertjadwal.php">Insert New Data</a>';
            }
            ?>
        </header>
        <main>
            <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    <form action="" method="post">
                        ...
                        <tbody>
                            <?php
                            $query = "SELECT * FROM jadwal"; // Ganti dengan nama tabel yang sesuai
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo "<tr>";
                                    if ($data['role'] == 'admin') {
                                        echo "<td><input type='checkbox' name='deleteId[]' value='" . $row['id_jadwal'] . "'></td>";
                                    }
                                    echo "<td>" . $row['id_jadwal'] . "</td>";
                                    echo "<td>" . $row['id_stasiunasal'] . "</td>";
                                    echo "<td>" . $row['id_stasiuntujuan'] . "</td>";
                                    echo "<td>" . $row['id_kereta'] . "</td>";
                                    echo "<td>" . $row['jam_berangkat'] . "</td>";
                                    echo "<td>" . $row['jam_sampai'] . "</td>";
                                    echo "<td>" . $row['tanggal'] . "</td>";
                                    if ($data['role'] == 'admin') {
                                        echo "<td><a href='form/formupdatejadwal.php?id=" . $row['id_jadwal'] . "' class='btn btn-warning btn-sm'>Edit</a> ";
                                    }
                                    echo "</tr>";
                                }
                            } else {
